...
def order_delete(O_id):
    c.execute(''' DELETE FROM Orders WHERE O_id = ?''', (O_id,))
    conn.commit()
...
def admin():
    ...
    if choice == "Orders":
            ...
            menu = ["View", "Delete"]  # Remove unnecessary options
            choice = st.sidebar.selectbox("Menu", menu)
            ...
            if choice == "View":
                ...
                order_id_to_delete = st.text_area("ID")
                if st.button(label="Delete"):
                    order_delete(order_id_to_delete)
                    st.success(f"Order {order_id_to_delete} deleted successfully.")
    elif choice == "About":
        st.subheader("DBMS Mini Project")
        st.subheader("By Sandra B S, Kezia Michelle George  ,Nandika Prince  & Sneha CM")
